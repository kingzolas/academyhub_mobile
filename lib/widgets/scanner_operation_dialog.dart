// lib/widgets/scanner_operation_dialog.dart
...
Future<T?> showScannerOperationDialog<T>({
  required BuildContext context,
  required Future<T> Function() operation,
  String loadingTitle = 'Processando...',
  String loadingMessage = 'Aguarde enquanto a operação é concluída.',
  String successTitle = 'Concluído!',
  String successMessage = 'Operação realizada com sucesso.',
  String? loadingDetail,
  Duration minimumLoadingDuration = const Duration(milliseconds: 1200),
  ...
  return showDialog<T>(
    ...
    builder: (_) => _ScannerOperationDialog<T>(
      operation: operation,
...
class _ScannerOperationDialog<T> extends StatefulWidget {
  ...
  @override
  State<_ScannerOperationDialog<T>> createState() =>
      _ScannerOperationDialogState<T>();
}

enum _ScannerDialogPhase { loading, success, error }

class _ScannerOperationDialogState<T> extends State<_ScannerOperationDialog<T>>
    with TickerProviderStateMixin {
  late final AnimationController _pulseController;
  late final AnimationController _dotsController;

  _ScannerDialogPhase _phase = _ScannerDialogPhase.loading;
  ...
  Future<void> _runOperation() async {
    ...
    try {
      _result = await widget.operation();
      ...
      if (!mounted) return;
      setState(() {
        _phase = _ScannerDialogPhase.success;
      });
      ...
      if (!mounted) return;
      setState(() {
        _phase = _ScannerDialogPhase.error;
  ...
  Widget build(BuildContext context) {
    ...
    final palette = _ScannerDialogPalette(isDark: isDark);
    final indicatorColor = switch (_phase) {
      _ScannerDialogPhase.loading => palette.accentPurple,
      _ScannerDialogPhase.success => palette.accentGreen,
      _ScannerDialogPhase.error => palette.accentRed,
    };
      ...
      child: ConstrainedBox(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: indicatorColor.withOpacity(isDark ? 0.15 : 0.08),
                blurRadius: 36,
                spreadRadius: 8,
  ...
  Widget _buildBody({
    required Key key,
    required _ScannerDialogPalette palette,
    required Color indicatorColor,
  }) {
    switch (_phase) {
      case _ScannerDialogPhase.loading:
        return _LoadingBody(
          key: key,
          palette: palette,
          indicatorColor: indicatorColor,
      ...
      case _ScannerDialogPhase.success:
        return _SuccessBody(
          key: key,
          palette: palette,
          indicatorColor: indicatorColor,
      ...
      case _ScannerDialogPhase.error:
        return _ErrorBody(
          key: key,
          palette: palette,
          indicatorColor: indicatorColor,
          title: 'Erro na Análise',
          message: _errorMessage ?? 'Não foi possível ler o gabarito.',
        );
    }
  }
}
...
class _LoadingBody extends StatelessWidget {
  final _ScannerDialogPalette palette;
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(detail ?? 'Aguarde um momento...',
                        style: GoogleFonts.inter(
                            fontSize: 13.2.sp,
                            fontWeight: FontWeight.w600,
                            color: palette.title)),
                    SizedBox(height: 4.h),
                    Text('Por favor, não feche o aplicativo agora.',
                        style: GoogleFonts.inter(
                            fontSize: 11.8.sp,
                            fontWeight: FontWeight.w500,
                            color: palette.muted)),
...
class _ScannerDialogPalette {
  final bool isDark;
  _ScannerDialogPalette({required this.isDark});
  ...
  Color get accentPurple =>
      const Color(0xFFC8A2C8); // A cor primária do Scanner!
